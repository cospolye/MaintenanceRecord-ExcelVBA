...
Sub GetMaintenanceCountByYear()
    ...
    Dim LastRow As Long
    LastRow = Sheets("ΜΑΙΝΤResults").Cells(Rows.Count, 2).End(xlUp).Row
    
    For i = 2 To LastRow
        If Sheets("ΜΑΙΝΤResults").Cells(i, 2).Value = MachineSerialNumber And Year(Sheets("ΜΑΙΝΤResults").Cells(i, 4).Value) = SelectedYear Then
